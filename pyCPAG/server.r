...
shinyServer(
  ...
  function(input, output, session) {
    ...
    pyInDir <- "input"
    pyOutDir <- "output"
    pyUserComputeDir <- "userCompute"
    pyInDirUserCompute <- paste(pyInDir, "/", pyUserComputeDir, sep="")
    pyOutDirUserCompute <- paste(pyOutDir, "/", pyUserComputeDir, sep="")
    inDir <- paste(pyDir, "/", pyInDir, sep="")
    outDir <- paste(pyDir, "/", pyOutDir, sep="")
    inDirUserCompute <- paste(inDir, "/", pyUserComputeDir, sep="")
    outDirUserCompute <- paste(outDir, "/", pyUserComputeDir, sep="")
      ...
      if(exploreVerifyParameterValues(source1, source2, pfactor1, pexp1, pfactor2, pexp2)) {
        ...
        if(length(which(dir(outDir, pattern=cpag, ignore.case=T)==cpag))>0) {
          ...
          if(length(kh)==0 & length(kn)>0) {
            pycmd2 <- paste("cd ", pyDir, "\n", pyexe, " main.py post_analysis --anno-ontology --anno-cols Trait1",
                            " --infile \"", pyOutDir, "/", cpag, "\" --outfile \"", pyOutDir, "/", cpag, "\"", sep="")
            cat(paste("\npycmd2:  ", pycmd2, "\n\n", sep=""), file=stderr())
          } else {
            pycmd2 <- ""
          }
          if(length(kh)==1 & length(kn)==1 | length(kn)==2) {
            pycmd3 <- paste("cd ", pyDir, "\n", pyexe, " main.py post_analysis --anno-ontology --anno-cols Trait2",
                            " --infile \"", pyOutDir, "/", cpag, "\" --outfile \"", pyOutDir, "/", cpag, "\"", sep="")
            cat(paste("\npycmd3:  ", pycmd3, "\n\n", sep=""), file=stderr())
          } else {
            pycmd3 <- ""
      ...
      if(f[["result"]]=="") {
        ...
        if(length(which(dir(inDirUserCompute, paste(f[["uGWASfile"]], ".csv", sep=""))==
          ...
          if(userComputeVerifyParameterValues(source2, pfactor2, pexp2)) {
            ...
            if(source2=="NHGRI") {
              pycmd2 <- paste("cd ", pyDir, "\n", pyexe, " main.py post_analysis --anno-ontology --anno-cols Trait2",
                              " --infile \"", pyOutDirUserCompute, "/", cpagFile, "\" --outfile \"",
                              pyOutDirUserCompute, "/", cpagFile, "\"", sep="")
              cat(paste("\npycmd2:  ", pycmd2, "\n\n", sep=""), file=stderr())
            } else {
              pycmd2 <- ""
            }
